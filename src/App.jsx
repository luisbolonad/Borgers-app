...
// cant = total combos vendidos; slotsSel ya tiene las cantidades TOTALES de cada opción
function ccComboConSel(combo,slotsSel,cant){return(combo.slots||[]).reduce((t,slot)=>{if(slot.tipo==="fijo_inv"){const it=inv.find(i=>i.id===slot.refId);return t+(it?.costo||0)*(slot.cantidad||1)*(cant||1);}if(slot.tipo==="fijo_rv"){const sub=rv.find(r=>r.id===slot.recetaId);return t+(sub?cc(sub)*(cant||1):0);}if(slot.tipo==="var_inv"){const sel=slotsSel?.[slot.nombre]||{};return t+(slot.opciones||[]).reduce((s,opt)=>{const totalQty=parseFloat(sel[opt.nombre]||0);if(!totalQty)return s;const it=inv.find(i=>i.id===opt.refId);return s+(it?.costo||0)*(opt.cantidad||1)*totalQty;},0);}if(slot.tipo==="var_rv"){const sel=slotsSel?.[slot.nombre]||{};return t+(slot.opciones||[]).reduce((s,opt)=>{const totalQty=parseFloat(sel[opt.nombre]||0);if(!totalQty)return s;const sub=rv.find(r=>r.id===opt.recetaId);return s+(sub?cc(sub)*totalQty:0);},0);}return t;},0);}
